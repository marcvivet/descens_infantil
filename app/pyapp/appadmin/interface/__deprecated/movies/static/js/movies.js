class PageSearchMoviesGallery extends PageRow {
    constructor(page) {
        super(null);
        ...
        this.div_x_content.appendChild(this.div_search);
    ...
    loadMoviesToDownload(title, callback = null) {
        this.loading = true;
                ...
                if ( request.length == 0 ) {
                    ...
                    for( var i = 0; i < request.length; ++i) {
                        ...
                        if (request[i].poster_path) {
                            ...
                            poster = '/movies/static/images/none.jpg';
                        }
    ...
    loadImage(data, poster) {
        ...
        image.id = data.id;
        image.src = poster;
        image.onload = ((imageParam, dataParam) => {
            return () => { this.imageLoaded(imageParam, dataParam); };
        })(image, data);
    }

    imageLoaded(image, data) {
        var loading_rect = document.getElementById('loading_rect');
        if (loading_rect) {
            this.div_gallery.removeChild(loading_rect);
        }
        ...
        var htmlResponse = '';
        htmlResponse += '<div class="col-gallery-info" id="gallery_' + image.id + '">\n';
        ...
        htmlResponse += '    <div class="col-gallery-info-data image_svg_canvas">\n';
        htmlResponse += '        <h2>' + data.title + '</h2>\n';
        htmlResponse += '        <ul>\n';
        
        htmlResponse += '          <li><b>Vote Average:</b> ' + data.vote_average + '</li>\n';
        htmlResponse += '          <li><b>Release Date:</b> ' + data.release_date + '</li>\n';
        htmlResponse += '          <li><b>Overview:</b> ' + data.overview + '</li>\n';
...
function initMovieGallery(page_type) {
    var title = null;
    switch (page_type) {
        case 'download':
    ...
    main_page = new Page(title);
    page_search_movies = new PageSearchMoviesGallery(main_page);
  
    // page_modal = new PageModalAnnotationDetails(main_page, page_row_gallery);
  
    main_page.addPageRow(page_search_movies);
...
function searchMovie() {
    ...
    page_search_movies.searchMovie(movie_title)
}
